 ...
 */
import type { NwsGrid, ParkLike, WeatherDay, WeatherHour, WeatherPayload } from "./types.ts";
...
export interface NwsWeatherOptions extends NwsFetchOptions {
  /**
   * Fetch the hourly grid too (default true). Basic-coverage parks only need the daily
   * periods, and skipping the second call halves the request count for a statewide run.
   */
  hourly?: boolean;
}

export async function fetchNwsWeather(
  park: Pick<ParkLike, "lat" | "lng" | "nws_grid">,
  fetchedAt: string,
  opts: NwsWeatherOptions = {},
  ...
  const [forecast, hourly] = await Promise.all([
    fetchForecast(grid, opts),
    wantHourly ? fetchHourly(grid, opts) : Promise.resolve(null),
  ]);
